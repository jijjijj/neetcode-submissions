...
private:
    ...
    double time(int pos_a, int target, int speed) {
        if (!speed) return 2000;

        const double delta = target - pos_a;
...
public:
    int carFleet(int target, vector<int>& position, vector<int>& speed) {
        ...
        double to_end = time(cars[0].pos, target, cars[0].speed);
        int ref_car = 0;
        int fleets = 1;
        ...
        for (size_t i = 1; i < n; ++i) {
            ...
            const bool will_catch = (cars[i].speed - cars[ref_car].speed) *
                to_end >= (cars[ref_car].pos - cars[i].pos);
